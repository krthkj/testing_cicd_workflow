 ...
 * where dist[i] describes the distance (in kilometers) of
 * the ith train ride.
 *
 * Each train can only depart at an integer hour, so you may
 * need to wait in between each train ride.
 *
 * For example, if the 1st train ride takes 1.5 hours, you must
 * wait for an additional 0.5 hours before you can depart on
 * the 2nd train ride at the 2 hour mark.
 *
 * Return the minimum positive integer speed (in kilometers per hour)
 * that all the trains must travel at for you to reach the office
 ...
 * - Since we are already at an integer hour, we depart immediately
 *   at the 1 hour mark. The second train takes 3/1 = 3 hours.
 * - Since we are already at an integer hour, we depart immediately
 *   at the 4 hour mark. The third train takes 2/1 = 2 hours.
...
{
bool timeRequired(std::vector<int>& dist, double hour, int speed);
int solution_1(std::vector<int>& dist, double hour);
} // namespace problem_1870
...
{
    return problem_1870::solution_1(dist, hour);
} // namespace leetcode

/***********************************************************
 * Implementation: Variations of Solution
 ***********************************************************/
bool problem_1870::timeRequired(std::vector<int>& dist, double hour, int speed)
{
    double travelTime{0.0};
    for (auto d : dist) {
        travelTime = std::ceil(travelTime);
        travelTime += ((double)d) / speed;
        if (hour < travelTime)
            return false;
    }
    return true;
...
int problem_1870::solution_1(std::vector<int>& dist, double hour)
{
    ...
    while (left < right) {
        ...
        if (problem_1870::timeRequired(dist, hour, mid))
            right = mid;
        else
            left = mid + 1;
    }
